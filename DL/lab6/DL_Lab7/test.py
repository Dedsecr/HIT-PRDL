...
def parse_args():
    ...
    parser.add_argument('--model_dir',
                        default='models/sigma25',
                        help='directory of the model')
    parser.add_argument('--model_name',
                        default='model_050.pth',
                        type=str,
                        help='the model name')
...
def test():
    for set_cur in args.set_names:
        ...
        if args.save_result:
            save_result(np.hstack((psnr_list, ssim_list)),
                        path=os.path.join(args.result_dir, set_cur,
                                          'results.txt'))
        print('[Dataset: %s] PSNR = %.2fdB, SSIM = %.4f' %
